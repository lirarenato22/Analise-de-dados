...
df_adicionado_white_ideology_renomeado_obama <- df_selecionado %>%
  mutate(white = dem_raceeth == "1. White non-Hispanic", ideology = case_when(ftgr_cons <= 33 ~ "liberal", ftgr_cons >= 34 & ftgr_cons <= 66 ~ "centro", ftgr_cons >= 67 & ftgr_cons <= 100 ~ "conservador"), obama_vote = recode(obama_vote, "1" = "Sim", "0" = "não"))
